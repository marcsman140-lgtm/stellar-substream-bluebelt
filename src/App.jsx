...
const TIERS = [
  { id: 'basic', name: 'Basic Tier', price: '10 XLM', amount: 10, interval: 'Every 30 Days', desc: 'Standard Continuous Protocol Stream' },
  { id: 'pro', name: 'Pro Tier', price: '50 XLM', amount: 50, interval: 'Every 30 Days', desc: 'High-Priority Automated Bandwidth & Analytics' },
  { id: 'enterprise', name: 'Enterprise Tier', price: '100 XLM', amount: 100, interval: 'Every 30 Days', desc: 'Dedicated Webhooks, SLA & Custom Corridors' }
];

const INITIAL_HISTORY = [
  { date: '2026-08-01', amount: '50 XLM', tier: 'Pro Tier', status: 'Settled', tx: 'f8a1c94be23d45fa108bc54329ef01a87b64c20e' },
  { date: '2026-08-08', amount: '50 XLM', tier: 'Pro Tier', status: 'Settled', tx: '9e4d210ae67b34fc8901ad238719bc54ef32901a' },
  { date: '2026-08-15', amount: '10 XLM', tier: 'Basic Tier', status: 'Settled', tx: '2a7b63f019ed48ab7610ce3982410a8b94ec1720' },
];
...
function App() {
  ...
  useEffect(() => {
    async function checkFreighter() {
      try {
        ...
        console.log('Freighter auto-connect check:', err);
      }
    }
    checkFreighter();
  }, []);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw new Error('INSUFFICIENT_BALANCE: Account not funded on Stellar Testnet. Use the Faucet button to fund it.');
        }
      }
      ...
      if (rpc.Api.isSimulationSuccess(sim)) {
        ...
        console.warn('Simulation notice (using base tx):', sim.error);
        preparedTx = tx;
      }
      ...
      while (attempts < 20 && !confirmed) {
        ...
        try {
          ...
          if (pollErr.message && pollErr.message.startsWith('CONTRACT_ERROR')) throw pollErr;
        }
        attempts++;
      }
      ...
      // Update Payment History with new on-chain subscription
      const newEntry = {
  ...
  const submitFeedback = () => {
    alert(`Thank you! Your ${rating}-star feedback for SubStream V2 has been recorded.`);
  ...
  if (currentView === 'landing') {
    return (
      <div className="landing-container animate-fade-in">
        <div className="glass-card landing-hero">
          <div className="landing-badge">Level 5 Blue Belt Project</div>
          <h1 className="landing-title">SubStream Protocol <span className="v2-badge">V2</span></h1>
          <p className="landing-subtitle">The first decentralized recurring payment & subscription streaming protocol on the Stellar Network.</p>
          
          <div className="pitch-section">
            <div className="pitch-box">
              <h3>🚨 The Problem</h3>
              <p>Legacy payment gateways extract 3–5% in fees, suffer from 7–15% involuntary churn, and lock users into centralized intermediaries. Web3 users are forced to manually sign recurring transactions every month or grant dangerous unlimited approvals.</p>
            </div>
            
            <div className="pitch-box">
              <h3>💡 The Solution</h3>
              <p>SubStream V2 utilizes Soroban smart contracts for time-locked interval authorizations. Enjoy multi-tier recurring value streams with zero-custody, sub-second finality, micro-cent gas fees, and instant sovereign revocation.</p>
            </div>
          </div>

          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="btn-launch" onClick={() => setCurrentView('app')}>
              Launch SubStream V2 dApp 🚀
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="app-container">
      <div className="glass-card animate-fade-in">
        <header className="header">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <button 
              ...
              style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#94a3b8', borderRadius: '6px', padding: '4px 10px', fontSize: '0.8rem', cursor: 'pointer' }}
            >
              ← Back to Pitch
            </button>
            <span className="v2-badge">Level 5 Scaled V2</span>
          </div>
          <h1>SubStream <span className="v2-badge">V2</span></h1>
          <p>Decentralized Recurring Payments on Stellar Soroban</p>
        ...
        {walletAddress && (
          <div className="tabs">
            <button className={`tab ${activeTab === 'subscribe' ? 'active' : ''}`} onClick={() => setActiveTab('subscribe')}>Subscription</button>
        ...
        {activeTab === 'subscribe' ? (
            ...
            {!walletAddress ? (
              ...
              <div className="animate-fade-in">
                ...
                {!isSubscribed ? (
                  ...
                  >
                    {isSubscribing ? `Authorizing Contract (${txStatus || 'INVOKING'})...` : `Subscribe to ${TIERS.find(t=>t.id===selectedTier)?.name}`}
                  </button>
                ) : (
                  <div style={{ textAlign: 'center' }}>
                    <button className="btn-primary" disabled style={{ background: '#10b981', width: '100%' }}>
                      ✓ Active Subscription: {TIERS.find(t=>t.id===selectedTier)?.name} ({TIERS.find(t=>t.id===selectedTier)?.price})
                    </button>
                    {txHash && (
          ...
          <div className="history-container animate-fade-in">
            <h3>Recent Automated Payments & Streams</h3>
            {paymentHistory.map((hist, i) => (
              <div key={i} className="history-row">
                <div>
                  <strong>{hist.date} — {hist.tier}</strong>
                  <div className="tx-hash">
                    {hist.tx.length > 20 ? (
                      <a
                        href={`https://stellar.expert/explorer/testnet/tx/${hist.tx}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{ color: '#94a3b8', textDecoration: 'none' }}
                      >
                        {hist.tx.substring(0, 10)}...{hist.tx.substring(hist.tx.length - 8)} ↗
                      </a>
                    ) : hist.tx}
                  </div>
                </div>
                <div className="history-right">
                  <span className="history-amount">-{hist.amount}</span>
        ...
        {showFeedback && (
          <div className="feedback-modal animate-fade-in">
            <p>How was your V2 onboarding experience?</p>
            <div className="rating-stars">
              {[1, 2, 3, 4, 5].map(star => (
                <span 
                  key={star} 
                  onClick={() => setRating(star)}
                  style={{ color: star <= rating ? '#fbbf24' : '#4b5563' }}
                >
                  ★
                </span>
              ))}
